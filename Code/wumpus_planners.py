...
class PlanRouteProblem(search.Problem):
	...
	explore = self.goals
	dist = [manhattan_distance_with_heading(node.state,e) for e in explore]
        return min(dist)
...
##################################################################################################
#												 #
#  Headings are represented as integers such that : North = 0, West = 1, South = 2 and East = 3  #
#												 #
##################################################################################################
	...
	s1 = state[1]
	s2 = state[2]       
        if s2 == 0 and (s0, s1 + 1) in self.allowed:
            return ['Forward','TurnRight','TurnLeft']
        if s2 == 1 and (s0 - 1, s1) in self.allowed:
            return ['Forward','TurnRight','TurnLeft']
        if s2 == 2 and (s0, s1 - 1) in self.allowed:
            return ['Forward','TurnRight','TurnLeft']
        if s2 == 3 and (s0 + 1, s1) in self.allowed:
            return ['Forward','TurnRight','TurnLeft']
...
#########################################################################
#		Forward action leading to next state       		#
#########################################################################

        if action == 'Forward' and s2 == 0:
            next_state = (s0, s1 + 1, s2)
	if action == 'Forward' and s2 == 1:
            next_state = (s0 - 1, s1, s2)
	if action == 'Forward' and s2 == 2:
            next_state = (s0, s1 - 1, s2)
	if action == 'Forward' and s2 == 3:
            next_state = (s0 + 1, s1, s2)

#########################################################################
#		Right turn leading to next state       		#
#########################################################################

	if action == 'TurnRight' and s2 == 0:
            next_state = (s0, s1, 3)
	if action == 'TurnRight' and s2 == 1:
            next_state = (s0, s1, 0)
	if action == 'TurnRight' and s2 == 2:
            next_state = (s0, s1, 1)
	if action == 'TurnRight' and s2 == 3:
            next_state = (s0, s1, 2)

#########################################################################
#		Left turn leading to next state       		#
#########################################################################

        if action == 'TurnLeft' and s2 == 0:
            next_state = (s0, s1, 1)
	if action == 'TurnLeft' and s2 == 1:
            next_state = (s0, s1, 2)
	if action == 'TurnLeft' and s2 == 2:
            next_state = (s0, s1, 3)
	if action == 'TurnLeft' and s2 == 3:
            next_state = (s0, s1, 0)
	return next_state
	...
	return True if (state[0], state[1]) in self.goals else False
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        ...
        self.hunt = []
        for g in self.goals:
            for locs in self.allowed:
                if (g[0] == locs[0]) or (g[1] == locs[1]):
                    self.hunt.append(locs)

        dist = [manhattan_distance_with_heading(node.state, hunt) for hunt in self.hunt]
        return min(dist)
...
#################################################################################################
            ...
            if pos[0] == s0 and ((pos[1] > s1) and s2 == 0):
                return True
...
#################################################################################################
	    ...
	    if pos[1] == s1 and ((pos[0] < s0) and s2 == 1):
                return True            
...
#################################################################################################
            ...
            if pos[0] == s0 and ((pos[1] < s1) and s2 == 2):
                return True
...
#################################################################################################
            ...
            if  pos[1] == s1 and ((pos[0] > s0) and s2 == 3):
                return True
        return False
